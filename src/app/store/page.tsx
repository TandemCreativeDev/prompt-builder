...
export default function PromptStorePage() {
  const [prefixesData, setPrefixesData] = useState<PrefixesData>({ prefixes: [] });
  const [suffixesData, setSuffixesData] = useState<SuffixesData>({ suffixes: [] });
  const [phasesConfig, setPhasesConfig] = useState<PhasesConfig>({ phases: [] });
  const [phasePromptsMap, setPhasePromptsMap] = useState<Record<string, PhasePromptsData>>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await Promise.all(
          phasesData.phases.map(async (phase) => {
            const phasePromptResponse = await fetch(`/api/prompts/phases/${phase.id}`);
  ...
  const handleSelectPhasePrompt = (phasePrompt: PromptFragment, phaseId: string) => {
    toast.info(`Selected phase prompt: ${phasePrompt.id} for phase: ${phaseId}`);
    // In a real app, you would update your main builder state here
  };

  const handleUpdatePrefix = async (prefixId: string, newText: string, persistChange: boolean): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`/api/prompts/prefixes/${prefixId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleUpdateSuffix = async (suffixId: string, newText: string, persistChange: boolean): Promise<boolean> => {
    try {
      // For session-only updates, just update the local state
      if (!persistChange) {
      ...
      // For persisted updates, call the API
      const response = await fetch(`/api/prompts/suffixes/${suffixId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdatePhasePrompt = async (
    ...
    try {
      ...
      // For persisted updates, call the API
      const response = await fetch(`/api/prompts/phases/${phaseId}/${promptId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Error Loading Prompt Store</h1>
